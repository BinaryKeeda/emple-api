...
export const getUserExamSolution = async (req, res) => {
  try {
    ...
    const test = await Exam.findById(testId)
      .select(
        "sections.title isProtected userDetails sections._id sections.type sections.maxQuestion sections.maxTime sections.maxScore sections.questionPool sections.problemPool"
      )
      .lean();
    ...
    let testSolution = await ExamSolution.findOne({ testId, userId });
    ...
    if (!testSolution) {
      ...
      testSolution = await ExamSolution.create({
        testId,
        userId,
        testSnapshot,
      });
...
export const startExam = async (req, res) => {
  try {
    ...
    const { submissionId , passcode, testId } = req.body;
    if(!testId) return res.status(400).json({message:"Please provide testid" , success:false})
    const exam = await Exam.findById(testId);
    if(exam.isProtected && !passcode && exam.passcode != '') {
        return res.json({success:false, message: "Please enter the passcode"}).status(200);
    }
    if(exam.isProtected && (passcode != exam.passcode)) {
       return res.json({success:false, message: "Please enter the passcode"}).status(200);
    }
    ...
    if (updated.matchedCount === 0) {
      return res.status(404).json({ success:false , message: "No test solution found" });
    }
